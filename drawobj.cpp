...
CDrawObj::CDrawObj()
{
	// file_logger = spdlog::basic_logger_mt("file_logger", "logs/basic_log.txt");
}
...
void CDrawRect::Draw(CDC* pDC)
{
	...
	case circle:
		if (std::abs(rect.Height()) > std::abs(rect.Width()))
		{
			if (rect.top > rect.bottom)
				rect.top = rect.bottom + std::abs(rect.Width());
			else
				rect.top = rect.bottom - std::abs(rect.Width());
		}
		else
		{
			if (rect.left > rect.right)
				rect.left = rect.right + std::abs(rect.Height());
			else
				rect.left = rect.right - std::abs(rect.Height());
		}
		// CRect circle_rect(CPoint(0, 0), CSize(100, 100));
		pDC->Ellipse(rect);
		break;
	}
